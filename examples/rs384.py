...
def generate_rs384_jwks(public_key, kid, use, algorithm):
...
def main():
    ...
    algorithm = 'RS384'
    token = generate_jwt_token(private_key_pem, payload, 'RS384')

    # 构建JWKS配置
    timestamp = time.time()
    kid = str(timestamp)
    jwk = generate_rs384_jwks(public_key, kid, "sig", algorithm)
    jwks = {
        "keys": [jwk]
    }

    # 输出 token
    print("JWT Token(RS384): ", "\n", token)
