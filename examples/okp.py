...
def generate_okp_jwks(public_key, kid, use, algorithm):
    if use != "enc" and use != "sig":
        raise ValueError("Invalid use value")

    # 获取公钥的x值
    x_base64_url = base64.urlsafe_b64encode(public_key.public_bytes(
        encoding = serialization.Encoding.Raw,
        format = serialization.PublicFormat.Raw
    )).decode('utf-8')
    x_base64_url = x_base64_url.rstrip('=')
...
def main():
    # 创建rsa公私钥
    private_key, private_key_pem, public_key, public_key_pem = generate_ed_key()
    ...
    timestamp = time.time()
    kid = str(timestamp)
    jwk = generate_okp_jwks(public_key, kid, "sig", algorithm)
    jwks = {
        "keys": [jwk]
    }

    # 输出 token
    print("JWT Token(EdDSA): ", "\n", token)
