...
def generate_rs512_jwks(public_key, kid, use, algorithm):
...
def main():
    ...
    algorithm = 'RS512'
    token = generate_jwt_token(private_key_pem, payload, 'RS512')

    # 构建JWKS配置
    timestamp = time.time()
    kid = str(timestamp)
    jwk = generate_rs512_jwks(public_key, kid, "sig", algorithm)
    jwks = {
        "keys": [jwk]
    }

    # 输出 token
    print("JWT Token(RS512): ", "\n", token)
