...
def generate_hs384_token():
    # 设置你的密钥，这是一个随机生成的字符串，需要保存好，用于验证 token
    # HS384需要至少48字节的密钥（384位/8）
    secret_key = 'my test secret key for hs384 algorithm requires longer key'
...
def generate_hs384_jwks():
    token, base64_key = generate_hs384_token()
...
def main():
    token, jwks = generate_hs384_jwks()

    # 输出 token
    print("JWT Token(HS384): " + token + "\n")
